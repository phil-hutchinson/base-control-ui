...
### The plan is where design history lives

Code in this repository does not carry its own design record (see
CONTRIBUTING.md, "Comments"). The reasoning behind a decision — the options
considered, the trade-off taken, why an approach was rejected — belongs here,
in the plan, and nowhere else. Write it down properly: this document is the
only place a future reader will find it.
...
2. **Automated test** — a unit, component, or integration test that can be run
   with a single command. Use when the behavior is purely internal, or when a
   component test covers it more reliably than an eye.
...
### Check the rules document

If the story changes how the game is played, the plan must include a step that
updates `doc/ruleset/rules.md`, bumps its version and the matching
`RULES_VERSION` constant, and adds a `doc/ruleset/changelog.md` entry — and
that step comes **before** the steps implementing the change, since the
document is what those steps implement.
...
anything the README describes, or confirming no update is needed. The
`/update-readme` command automates this: it reviews the current branch diff and
updates `README.md` if warranted.
