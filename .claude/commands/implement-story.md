...
3. You are running inside the dev container. All toolchain commands run
   there; never suggest running anything on the host.
...
4. Ask the owner to look over the result: summarize what the story delivered,
   ...
   - Satisfied → sign-off. Report the final state (commits made, checks
     ...
     owner's; do not do it. If the story changed the ruleset, remind the
     owner to run `/tag-rules` after merging.
