...
/**
 * Whether the side to move has any legal action at all — a legal move with
 * any ship that has not yet moved this ply, or a legal attack target with
 * any of its ships — judged by the §6-only and §7-only layers, with no
 * awareness of §8.5's stranded-ship obligation. Used by the §5 pass guard,
 * which must keep working regardless of the obligation: the obligation only
 * ever binds when at least one ship has a legal §6 move, so a side that is
 * obliged always has an action available (the same reasoning
 * `sideToMoveHasLegalMove` already relies on, which this is built on top of).
 */
