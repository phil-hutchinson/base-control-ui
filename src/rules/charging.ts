...
// node charges next. If the shortfall is greater than the queue can cover
// — the one turn all four charged nodes run out at once — the remainder
// stays unfilled here; the direct fourth placement rules.md §8.2 describes
// for that case is added separately.
 ...
 * standing on it charges like any other. There is no draw here at all —
 * the order is decided entirely by `orderByPriorityDescending` — so
 * `state.randomSeed` is returned untouched.
 */
